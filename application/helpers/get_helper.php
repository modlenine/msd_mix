...
function valueFormat3($inputNumber)
{
    $conToDecimal = floatval($inputNumber);
    $stringafterDot = strstr($conToDecimal, ".");
    $decimalNumber = strlen($stringafterDot);
    ...
    if($decimalNumber == 0){
        ...
        $conNumber = number_format($inputNumber , 3);
    }
...
function getDataRun($m_code)
{
    ...
    details.d_user_finish,
    details.d_ecode_finish,
    details.d_deptcode_finish,
    details.d_datetime_finish
    FROM
    details
...
function getSubDetail_export($m_code , $d_code)
{
    $sql = gfn()->db->query("SELECT
    ...
    FROM
    sub_details
    WHERE sd_maincode = '$m_code' AND
    sd_detailcode = '$d_code'
    ");
...
function getRefVersion($m_code)
{
    $sql = gfn()->db->query("SELECT
    ref_version
    FROM reference WHERE ref_m_code = '$m_code' AND ref_type = 'Actual' GROUP BY ref_code ORDER BY ref_autoid DESC LIMIT 1
    ");
    return $sql;
}


function getdataTemplateDetail($template_mastercode , $columnname , $ref_code)
{
    if($template_mastercode != "" && $columnname != "" && $ref_code != ""){
        $sql = gfn()->db->query("SELECT ref_detail_name FROM reference WHERE ref_m_code = '$template_mastercode' AND ref_column_name = '$columnname' AND ref_code = '$ref_code' ORDER BY ref_linenum ASC ");
        return $sql;
    }
}


function getDetailMemo($m_code , $d_code)
{
    if($m_code != "" && $d_code != ""){
        $sql = gfn()->db->query("SELECT me_memo FROM memo WHERE me_maincode = '$m_code' AND me_detailcode = '$d_code' ");
        if($sql->num_rows() != 0 ){
            return $sql->row()->me_memo;
        }
    }
}
